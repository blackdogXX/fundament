...
public class FinService
{
    ...
    public async Task<Dictionary<string, double>> ByCategory(int? siteId = null) { var q = _ctx.Expenses.Where(e => e.UserId == Uid).AsQueryable(); if (siteId.HasValue) q = q.Where(e => e.SiteId == siteId.Value); return await q.Where(e => e.Category != null).GroupBy(e => e.Category.Name).ToDictionaryAsync(g => g.Key, g => g.Sum(e => e.Amount)); }
    public async Task<Dictionary<string, double>> BySubcategory(int? siteId = null) { var q = _ctx.Expenses.Where(e => e.UserId == Uid).AsQueryable(); if (siteId.HasValue) q = q.Where(e => e.SiteId == siteId.Value); return await q.Where(e => e.Subcategory != null).GroupBy(e => e.Subcategory.Name).ToDictionaryAsync(g => g.Key, g => g.Sum(e => e.Amount)); }
    public async Task<bool> ExpenseExists(Expense e) => await _ctx.Expenses.AnyAsync(x => x.UserId == Uid && x.Amount == e.Amount && x.Date == e.Date && x.SiteId == e.SiteId && x.Description == e.Description);
    public async Task<bool> IncomeExists(Income i) => await _ctx.Incomes.AnyAsync(x => x.UserId == Uid && x.Amount == i.Amount && x.Date == i.Date && x.SiteId == i.SiteId && x.Description == i.Description);
